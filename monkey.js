...
(() => {
  ...
  // <define:process.env>
  var define_process_env_default = { BUILD_TIME: "2022-12-22T05:19:13.975Z", TRANSLATE_SERVICE: "google", MOCK: "0", DEBUG: "0", TRANSLATE_TAMPERMONKEY: "1" };

  // browser/tampermonkey_fetch_polifill.js
  (function() {
    "use strict";
...
${r13.map((n21, s19) => `${s19 + 1}) ${n21.toString()}`).join(`
  ...
  var Logger = class {
    ...
    debug(...args) {
      if (this.#level <= 0 /* Debug */) {
        console.debug(colors_exports.dim(brandName + " DEBUG:"), ...args);
      }
    }
    info(...args) {
  ...
  function getEnv() {
    ...
    const env2 = define_process_env_default;
    return env2;
  }
  function isMonkey() {
    const env2 = getEnv();
    return env2.TRANSLATE_TAMPERMONKEY === "1";
  }
  ...
  // browser/tampermonkey_polyfill.ts
  var storageApi = {
    get: (key) => {
  ...
  var tampermonkey_polyfill_default = browser;

  // browser/browser.ts
  var env = getEnv();
  var browserAPI;
  if (env.TRANSLATE_TAMPERMONKEY === "1") {
    browserAPI = tampermonkey_polyfill_default;
  } else if (isDeno()) {
  ...
  function getEnvUserConfig() {
    ...
    const env2 = getEnv();
    if (env2.TRANSLATE_SECRET_TENCENT_SECRET_ID && env2.TRANSLATE_SECRET_TENCENT_SECRET_KEY) {
      const tencentAuthConfig = {
        secretId: env2.TRANSLATE_SECRET_TENCENT_SECRET_ID,
        secretKey: env2.TRANSLATE_SECRET_TENCENT_SECRET_KEY
      };
      defaultUserConfig.translationServices = {};
      defaultUserConfig.translationServices.tencent = tencentAuthConfig;
    }
    if (env2.TRANSLATE_SERVICE) {
      defaultUserConfig.translationService = env2.TRANSLATE_SERVICE;
    }
    if (env2.DEBUG === "1") {
  ...
  async function getConfig() {
    ...
    let finalBuildInConfig = {
      ...
      buildinConfigUpdatedAt: env2.BUILD_TIME
    };
    ...
    const mergedUserConfig = Object.assign({}, envUserConfig, userConfig);
  ...
  var _Tencent = class extends Translation {
    ...
    async signedRequest({
      ...
      const host2 = `${service}.tencentcloudapi.com`;
      const now = new Date();
  ...
  // utils/url_match.ts
  var scheme = "(\\*|http|https|file|ftp|ws|wss|data)";
  var host = "(\\*|(?:\\*\\.)?(?:[^*]+)(?:.*)?)?";
  var path = "(.*)";
  var schemeRegex = new RegExp("^" + scheme + "://");
  var regexString = "^" + scheme + "://" + host + "/" + path + "$";
  var regex2 = new RegExp(
    regexString
  );
  function parsePattern(pattern) {
    const isIncludeSscheme = schemeRegex.test(pattern);
    if (!isIncludeSscheme) {
      if (!pattern.includes("/")) {
        pattern = pattern + "/*";
      }
      ...
      const patternWithoutScheme = pattern.replace(schemeRegex, "");
      if (!patternWithoutScheme.includes("/")) {
        pattern = pattern + "/*";
      }
    }
    const match = regex2.exec(pattern);
    if (!match)
      return null;
    const scheme2 = match[1];
    const host2 = match[2];
    const path2 = match[3];
    if (!host2 && scheme2 !== "file")
      return null;
    return makeRegExp(scheme2, host2, path2);
  }
  function makeRegExp(scheme2, host2, path2) {
    let regex3 = "^";
    if (scheme2 === "*") {
      regex3 += "(http|https)";
    } else {
    ...
    if (host2) {
      if (host2 === "*") {
        ...
        if (host2.match(/^\*\./)) {
          regex3 += "[^/]*?";
          host2 = host2.substring(2);
        }
        regex3 += host2.replace(/\./g, "\\.").replace(/\*/g, "[^/]*");
      }
    }
    if (path2) {
      if (path2 === "*") {
  ...
  function getMatchedUrl(url, rawMatches) {
    ...
    if (matches && matches.length > 0) {
      const matched = matches.find((match) => {
        const isDomain = isValidHostname(match);
        if (isDomain) {
          return hostname === match;
        } else {
          const pattern = parsePattern(match);
          if (pattern) {
  ...
  main().catch((e18) => {
    if (e18) {
      log_default.error("translate page error", e18.name, e18.message, e18.details || "", e18);
    }
  });
})();
/*! *****************************************************************************
